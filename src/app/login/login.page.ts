...
export class LoginPage implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.logo_url = this.global.logoUrl;
  ...
  async loadAndStoreUserInfo(provider: string) {
    ...
    if (this.global.onlyLoyalty && this.global.loyaltyMode != "EXTERNAL_POINTS") this.router.navigate(["tabs/loyalty"]);
  }
  ...
  async openModalRegister(email: string, firstname: string, lastname: string, provider: EProvider, providerId: string) {
    const modal = await this.modalCtrl.create({
      ...
      componentProps: { emailInput: email, firstname, lastname, provider, id_in_provider: providerId, registerFunc: this.cadastro },
    });
